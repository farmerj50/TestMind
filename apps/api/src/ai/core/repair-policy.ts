...
export function classifyFailureContext(input: {
  ...
  // Only classify as login_selector_timeout when the failure message itself
  // mentions login selectors, OR when the spec has login patterns AND the
  // failing test title is clearly login-related. Checking specContent alone
  // is too broad — it fires for every test in a spec that happens to share
