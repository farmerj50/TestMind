...
import { getFrameworkDefinition } from "@testmind/core/framework-registry";
...
export default async function testmindRoutes(app: FastifyInstance): Promise<void> {
    ...
    for (const root of roots) {
      ...
      if (fs.existsSync(parent)) {
        if (projectId) {
          ...
          // Do NOT fall back to the parent folder when projectId is specified —
          // that would pull in generated specs from all other projects.
          continue;
        }
        return parent;
      }
  ...
  app.post("/suite/projects/:id/sync-from-generated", async (req, reply) => {
    try {
      ...
      // When the suite belongs to a specific project, only sync specs generated for that
      // project. Do NOT fall back to the user-level folder — that contains every project's
      // specs and causes cross-project bleed.
      if (targetSuite.projectId && !projectRoot) {
        return reply.code(404).send({
          error: `No generated specs found for this project (${adapterId}). Run generation for this project first, then sync.`,
        });
      }
  ...
  app.delete("/suite/projects/:id/folder", async (req, reply) => {
    try {
      ...
      const suite = await prisma.curatedSuite.findUnique({
        where: { id },
        select: { rootRel: true, name: true, projectId: true, project: { select: { ownerId: true } } },
      });

      let root: string | null = null;
      if (suite && suite.project.ownerId === userId) {
        const curatedSuiteObj: CuratedSuiteWithOwner = {
          id,
          name: suite.name ?? "",
        ...
        root = resolveCuratedRoot(curatedSuiteObj) ?? null;
