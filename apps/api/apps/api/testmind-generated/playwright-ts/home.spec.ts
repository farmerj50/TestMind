...
test("Page loads: /", async ({ page }) => {
  ...
  await test.step("1. Navigate to /", async () => {
    const response = await page.goto("/", { timeout: 60000 }); // Increased timeout to 60s
  ...
  await test.step("2. Ensure text \"TestMind AI\" is visible", async () => {
    ...
    await expect(locator).toBeVisible({ timeout: 30000 });  // Increased timeout for visibility checks
  });
