        ...
        onConfirm={async () => {
          ...
          if (deleteFiles) {
            try {
              ...
              // Remove specs and folder entries from UI state for the deleted folder
              updateSuiteFolderState(target.suiteId, (state) => {
                const normalized = normalizeFolderPath(target.folderPath);
                const nextSpecFolders = { ...state.specFolders };
                // Remove all specFolders entries inside the deleted folder
                Object.keys(nextSpecFolders).forEach((p) => {
                  ...
                  // Also remove entries whose key path itself is inside the deleted folder
                  const normalizedKey = normalizeFolderPath(p.replace(/\\/g, "/").split("/").slice(0, -1).join("/"));
                  if (normalizedKey === normalized || normalizedKey?.startsWith(`${normalized}/`)) delete nextSpecFolders[p];
                });
