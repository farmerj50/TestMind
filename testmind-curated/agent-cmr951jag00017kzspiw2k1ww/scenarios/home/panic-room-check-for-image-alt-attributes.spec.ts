...
test("Panic Room — check for image alt attributes", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — check for image alt attributes" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure locator img[alt] is visible", async () => {
    try {
    // Missing locator locators.img-alt on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Ensure locator img[alt] is visible");
    }
  });
});
