...
test("Panic Room — check for proper error handling on landing page", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — check for proper error handling on landing page" }, { type: "parameter", description: "page=/" });
  await test.step("1. Navigate to https://panic-room-front-end-production.up.railway.app/non-existent-page", async () => {
    try {
    await navigateTo(page, "/non-existent-page");
      await ensurePageIdentity(page, "/non-existent-page");
    } finally {
      await captureStepArtifact(page, testInfo, "1. Navigate to https://panic-room-front-end-production.up.railway.app/non-existent-page");
    }
  });
  await test.step("2. Ensure text \"404 Not Found\" is visible", async () => {
    try {
    {
      const rawText = "404 Not Found";
      if (/justicepath/i.test(rawText) && "/non-existent-page" !== "/") {
        await ensurePageIdentity(page, "/non-existent-page");
        return;
      }
      if (rawText.trim().toLowerCase() === "page") {
        await expect(page).toHaveURL(pathRegex("/non-existent-page"), { timeout: 15000 });
        await ensurePageIdentity(page, "/non-existent-page");
        return;
      }
      ...
      await captureStepArtifact(page, testInfo, "2. Ensure text \"404 Not Found\" is visible");
    }
  });
});
