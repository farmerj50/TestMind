...
test("Panic Room — check for proper use of headings", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — check for proper use of headings" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure locator h1 is visible", async () => {
    try {
    // Missing locator locators.h1 on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Ensure locator h1 is visible");
    }
  });
  await test.step("3. Ensure locator h2 is visible", async () => {
    try {
    // Missing locator locators.h2 on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "3. Ensure locator h2 is visible");
    }
  });
});
