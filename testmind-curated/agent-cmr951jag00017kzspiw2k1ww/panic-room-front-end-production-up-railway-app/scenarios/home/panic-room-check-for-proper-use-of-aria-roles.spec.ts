...
test("Panic Room — check for proper use of ARIA roles", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — check for proper use of ARIA roles" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure locator [role] is visible", async () => {
    try {
    // Missing locator locators.role on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Ensure locator [role] is visible");
    }
  });
});
