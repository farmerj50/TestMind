...
test("Panic Room — verify responsive design on landing page", async ({ page }, testInfo: TestInfo) => {
  ...
  await test.step("2. Ensure locator header is visible", async () => {
    try {
    // Missing locator locators.header on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Ensure locator header is visible");
    }
  });
  ...
  await test.step("4. Ensure locator header is visible", async () => {
    try {
    // Missing locator locators.header on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "4. Ensure locator header is visible");
    }
  });
});
