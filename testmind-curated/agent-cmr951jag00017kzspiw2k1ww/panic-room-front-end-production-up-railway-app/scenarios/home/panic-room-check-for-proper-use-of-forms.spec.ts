...
test("Panic Room — check for proper use of forms", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — check for proper use of forms" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure locator form is visible", async () => {
    try {
    // Missing locator locators.form on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Ensure locator form is visible");
    }
  });
});
