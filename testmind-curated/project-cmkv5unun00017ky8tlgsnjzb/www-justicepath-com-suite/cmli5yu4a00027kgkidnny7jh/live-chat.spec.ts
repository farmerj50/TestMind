...
test("Page loads: /live-chat", async ({ page }, testInfo: TestInfo) => {
  ...
  await test.step("1. Navigate to https://www.justicepathlaw.com/live-chat", async () => {
    ...
    await navigateTo(page, "/live-chat");
      ...
      await captureStepArtifact(page, testInfo, "1. Navigate to https://www.justicepathlaw.com/live-chat");
    }
  });
...
test("Form submits – /live-chat", async ({ page }, testInfo: TestInfo) => {
  ...
  await test.step("2. Fill [name='Jurisdiction (e.g., Atlanta, GA)'], #Jurisdiction (e.g., Atlanta, GA)", async () => {
    ...
    // Missing locator fields.name-jurisdiction-e-g-atlanta-ga-jurisdiction-e-g-atlanta-ga on /live-chat; add it to shared locators and rerun generation.
      ...
      await captureStepArtifact(page, testInfo, "2. Fill [name='Jurisdiction (e.g., Atlanta, GA)'], #Jurisdiction (e.g., Atlanta, GA)");
    }
  });
...
test("Navigate /live-chat → /pricing", async ({ page }, testInfo: TestInfo) => {
...
test("Navigate /live-chat → /login", async ({ page }, testInfo: TestInfo) => {
...
test("Navigate /live-chat → /signup", async ({ page }, testInfo: TestInfo) => {
