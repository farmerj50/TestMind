...
test("Page loads: /select-plan", async ({ page }, testInfo: TestInfo) => {
  ...
  await test.step("1. Navigate to https://www.justicepathlaw.com/select-plan", async () => {
    try {
    await navigateTo(page, "/select-plan");
      await ensurePageIdentity(page, "/select-plan");
    } finally {
      await captureStepArtifact(page, testInfo, "1. Navigate to https://www.justicepathlaw.com/select-plan");
    }
  });
  await test.step("2. Ensure text \"JusticePath — Accessible Legal Help\" is visible", async () => {
    try {
    {
      ...
      if (/justicepath/i.test(rawText) && "/select-plan" !== "/") {
        await ensurePageIdentity(page, "/select-plan");
        return;
      }
      if (rawText.trim().toLowerCase() === "page") {
        await expect(page).toHaveURL(pathRegex("/select-plan"), { timeout: 15000 });
        await ensurePageIdentity(page, "/select-plan");
        return;
      }
...
test("Navigate /select-plan → /live-chat", async ({ page }, testInfo: TestInfo) => {
...
test("Navigate /select-plan → /pricing", async ({ page }, testInfo: TestInfo) => {
...
test("Navigate /select-plan → /login", async ({ page }, testInfo: TestInfo) => {
...
test("Navigate /select-plan → /signup", async ({ page }, testInfo: TestInfo) => {
