...
test("Page loads: /", async ({ page }, testInfo: TestInfo) => {
  ...
  await test.step("1. Navigate to https://www.justicepathlaw.com/", async () => {
    try {
    await navigateTo(page, "/");
      await ensurePageIdentity(page, "/");
    } finally {
      await captureStepArtifact(page, testInfo, "1. Navigate to https://www.justicepathlaw.com/");
    }
  });
...
test("Navigate / → /live-chat", async ({ page }, testInfo: TestInfo) => {
...
test("Navigate / → /pricing", async ({ page }, testInfo: TestInfo) => {
...
test("Navigate / → /select-plan", async ({ page }, testInfo: TestInfo) => {
  ...
  await test.step("2. Navigate to /select-plan", async () => {
    ...
    await navigateTo(page, "/select-plan");
      ...
      await captureStepArtifact(page, testInfo, "2. Navigate to /select-plan");
    }
  });
  await test.step("3. Ensure text \"select-plan\" is visible", async () => {
    try {
    {
      const rawText = "select-plan";
      if (/justicepath/i.test(rawText) && "/" !== "/") {
        await ensurePageIdentity(page, "/");
        return;
      }
      ...
      await captureStepArtifact(page, testInfo, "3. Ensure text \"select-plan\" is visible");
    }
  });
});
...
test("Navigate / → /case-type-selection", async ({ page }, testInfo: TestInfo) => {
  ...
  await test.step("2. Navigate to /case-type-selection", async () => {
    ...
    await navigateTo(page, "/case-type-selection");
      ...
      await captureStepArtifact(page, testInfo, "2. Navigate to /case-type-selection");
    }
  });
  await test.step("3. Ensure text \"case-type-selection\" is visible", async () => {
    try {
    {
      const rawText = "case-type-selection";
      if (/justicepath/i.test(rawText) && "/" !== "/") {
        await ensurePageIdentity(page, "/");
        return;
      }
      ...
      await captureStepArtifact(page, testInfo, "3. Ensure text \"case-type-selection\" is visible");
    }
  });
});
