...
test("Page loads: /pricing", async ({ page }, testInfo: TestInfo) => {
  ...
  await test.step("1. Navigate to https://www.justicepathlaw.com/pricing", async () => {
    try {
    await navigateTo(page, "/pricing");
      await ensurePageIdentity(page, "/pricing");
    } finally {
      await captureStepArtifact(page, testInfo, "1. Navigate to https://www.justicepathlaw.com/pricing");
    }
  });
  await test.step("2. Ensure text \"JusticePath — Accessible Legal Help\" is visible", async () => {
    try {
    {
      ...
      if (/justicepath/i.test(rawText) && "/pricing" !== "/") {
        await ensurePageIdentity(page, "/pricing");
        return;
      }
      if (rawText.trim().toLowerCase() === "page") {
        await expect(page).toHaveURL(pathRegex("/pricing"), { timeout: 15000 });
        await ensurePageIdentity(page, "/pricing");
        return;
      }
...
test("Navigate /pricing → /live-chat", async ({ page }, testInfo: TestInfo) => {
...
test("Navigate /pricing → /login", async ({ page }, testInfo: TestInfo) => {
...
test("Navigate /pricing → /signup", async ({ page }, testInfo: TestInfo) => {
