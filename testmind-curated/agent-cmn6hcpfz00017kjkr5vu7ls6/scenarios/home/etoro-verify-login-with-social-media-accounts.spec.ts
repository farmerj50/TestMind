...
test("eToro — verify login with social media accounts", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/home" }, { type: "story", description: "eToro — verify login with social media accounts" }, { type: "parameter", description: "page=/home" });
  ...
  await test.step("2. Click button:has-text(\"Sign in with Google\")", async () => {
    try {
    // Missing locator locators.button-has-text-sign-in-with-google on /home; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Click button:has-text(\"Sign in with Google\")");
    }
  });
});
