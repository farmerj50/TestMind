...
test("eToro — verify login form validation for empty fields", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/home" }, { type: "story", description: "eToro — verify login form validation for empty fields" }, { type: "parameter", description: "page=/home" });
  ...
  await test.step("2. Click button:has-text(\"Sign in\")", async () => {
    try {
    // Missing locator locators.button-has-text-sign-in on /home; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Click button:has-text(\"Sign in\")");
    }
  });
});
