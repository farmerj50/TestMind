...
test("eToro — verify cookie consent buttons", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/home" }, { type: "story", description: "eToro — verify cookie consent buttons" }, { type: "parameter", description: "page=/home" });
  ...
  await test.step("2. Click button:has-text(\"Accept All\")", async () => {
    try {
    // Missing locator locators.button-has-text-accept-all on /home; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Click button:has-text(\"Accept All\")");
    }
  });
});
