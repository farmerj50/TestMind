...
test("eToro — verify checkbox functionality for terms and conditions", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/home" }, { type: "story", description: "eToro — verify checkbox functionality for terms and conditions" }, { type: "parameter", description: "page=/home" });
  ...
  await test.step("2. Click ot-group-id-C0002", async () => {
    try {
    // Missing locator locators.ot-group-id-c0002 on /home; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Click ot-group-id-C0002");
    }
  });
});
