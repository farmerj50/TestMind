...
// Auto-generated for page /home 1 test(s)

test("eToro — verify forgot password link functionality", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/home" }, { type: "story", description: "eToro — verify forgot password link functionality" }, { type: "parameter", description: "page=/home" });
  await test.step("1. Navigate to https://www.etoro.com/home", async () => {
    try {
    await navigateTo(page, "/home");
      await ensurePageIdentity(page, "/home");
    } finally {
      await captureStepArtifact(page, testInfo, "1. Navigate to https://www.etoro.com/home");
    }
  });
  await test.step("2. Click https://www.etoro.com/accounts/forgot-password", async () => {
    try {
    // Missing locator locators.nav.accounts-forgot-password on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Click https://www.etoro.com/accounts/forgot-password");
    }
  });
});
