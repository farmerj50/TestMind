...
test("Panic Room — check for accessibility compliance", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — check for accessibility compliance" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure text \"Accessibility\" is visible", async () => {
    try {
    {
      const rawText = "Accessibility";
      ...
      await captureStepArtifact(page, testInfo, "2. Ensure text \"Accessibility\" is visible");
    }
  });
});
