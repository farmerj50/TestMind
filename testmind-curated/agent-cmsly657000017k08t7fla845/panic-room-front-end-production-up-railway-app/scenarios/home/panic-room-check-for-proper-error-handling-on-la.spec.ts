...
test("Panic Room — check for proper error handling on landing page", async ({ page }, testInfo: TestInfo) => {
  ...
  await test.step("1. Navigate to https://panic-room-front-end-production.up.railway.app/nonexistent", async () => {
    try {
    await navigateTo(page, "/nonexistent");
      await ensurePageIdentity(page, "/nonexistent");
    } finally {
      await captureStepArtifact(page, testInfo, "1. Navigate to https://panic-room-front-end-production.up.railway.app/nonexistent");
    }
  });
  await test.step("2. Ensure text \"404 Not Found\" is visible", async () => {
    try {
    {
      ...
      if (/justicepath/i.test(rawText) && "/nonexistent" !== "/") {
        await ensurePageIdentity(page, "/nonexistent");
        return;
      }
      if (rawText.trim().toLowerCase() === "page") {
        await expect(page).toHaveURL(pathRegex("/nonexistent"), { timeout: 15000 });
        await ensurePageIdentity(page, "/nonexistent");
        return;
      }
