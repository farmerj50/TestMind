...
test("Panic Room — verify page title on landing page", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — verify page title on landing page" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure text \"Panic Room\" is visible", async () => {
    try {
    {
      const rawText = "Panic Room";
      ...
      await captureStepArtifact(page, testInfo, "2. Ensure text \"Panic Room\" is visible");
    }
  });
});
