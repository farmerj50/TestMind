...
test("Panic Room — ensure proper loading of external resources on landing page", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — ensure proper loading of external resources on landing page" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure locator link[rel='stylesheet'] is visible", async () => {
    try {
    // Missing locator locators.link-rel-stylesheet on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Ensure locator link[rel='stylesheet'] is visible");
    }
  });
  await test.step("3. Ensure locator script[src] is visible", async () => {
    try {
    // Missing locator locators.script-src on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "3. Ensure locator script[src] is visible");
    }
  });
});
