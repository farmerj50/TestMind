...
test("Panic Room — ensure no sensitive data exposure on landing page", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — ensure no sensitive data exposure on landing page" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure text \"password\" is visible", async () => {
    try {
    {
      ...
      await captureStepArtifact(page, testInfo, "2. Ensure text \"password\" is visible");
    }
  });
  await test.step("3. Ensure text \"secret\" is visible", async () => {
    try {
    {
      const rawText = "secret";
      if (/justicepath/i.test(rawText) && "/" !== "/") {
        await ensurePageIdentity(page, "/");
        return;
      }
      ...
      await captureStepArtifact(page, testInfo, "3. Ensure text \"secret\" is visible");
    }
  });
});
