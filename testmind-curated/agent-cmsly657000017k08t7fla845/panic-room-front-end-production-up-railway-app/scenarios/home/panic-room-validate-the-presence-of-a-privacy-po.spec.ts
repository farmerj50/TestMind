...
test("Panic Room — validate the presence of a privacy policy link on landing page", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — validate the presence of a privacy policy link on landing page" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure locator a[href*='privacy'] is visible", async () => {
    try {
    // Missing locator locators.a-href-privacy on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Ensure locator a[href*='privacy'] is visible");
    }
  });
});
