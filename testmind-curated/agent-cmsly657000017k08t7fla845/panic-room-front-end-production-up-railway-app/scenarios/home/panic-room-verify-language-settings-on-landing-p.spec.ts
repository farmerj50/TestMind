...
test("Panic Room — verify language settings on landing page", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — verify language settings on landing page" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure text \"lang\" is visible", async () => {
    try {
    {
      const rawText = "lang";
      ...
      await captureStepArtifact(page, testInfo, "2. Ensure text \"lang\" is visible");
    }
  });
});
