...
test("Panic Room — validate security headers on landing page", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — validate security headers on landing page" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure text \"X-Content-Type-Options\" is visible", async () => {
    try {
    {
      const rawText = "X-Content-Type-Options";
      ...
      await captureStepArtifact(page, testInfo, "2. Ensure text \"X-Content-Type-Options\" is visible");
    }
  });
  await test.step("3. Ensure text \"X-Frame-Options\" is visible", async () => {
    try {
    {
      const rawText = "X-Frame-Options";
      if (/justicepath/i.test(rawText) && "/" !== "/") {
        await ensurePageIdentity(page, "/");
        return;
      }
      ...
      await captureStepArtifact(page, testInfo, "3. Ensure text \"X-Frame-Options\" is visible");
    }
  });
});
