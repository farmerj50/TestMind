...
test("Landing - verify primary content remains visible", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Landing - verify primary content remains visible" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure locator main, [role='main'], body is visible", async () => {
    try {
    // Missing locator locators.main-role-main-body on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Ensure locator main, [role='main'], body is visible");
    }
  });
});
