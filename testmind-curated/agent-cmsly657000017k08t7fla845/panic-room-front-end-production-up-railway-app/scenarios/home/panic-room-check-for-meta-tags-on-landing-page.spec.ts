...
test("Panic Room — check for meta tags on landing page", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — check for meta tags on landing page" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure locator meta[name='description'] is visible", async () => {
    try {
    // Missing locator locators.meta-name-description on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Ensure locator meta[name='description'] is visible");
    }
  });
  await test.step("3. Ensure locator meta[name='viewport'] is visible", async () => {
    try {
    // Missing locator locators.meta-name-viewport on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "3. Ensure locator meta[name='viewport'] is visible");
    }
  });
});
