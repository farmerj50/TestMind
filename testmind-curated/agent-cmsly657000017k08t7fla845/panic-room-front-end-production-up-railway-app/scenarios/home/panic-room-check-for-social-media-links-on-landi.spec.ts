...
test("Panic Room — check for social media links on landing page", async ({ page }, testInfo: TestInfo) => {
  test.info().annotations.push({ type: "parentSuite", description: "Testmind Generated Suite" }, { type: "suite", description: "/" }, { type: "story", description: "Panic Room — check for social media links on landing page" }, { type: "parameter", description: "page=/" });
  ...
  await test.step("2. Ensure locator a[href*='facebook.com'] is visible", async () => {
    try {
    // Missing locator locators.a-href-facebook-com on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "2. Ensure locator a[href*='facebook.com'] is visible");
    }
  });
  await test.step("3. Ensure locator a[href*='twitter.com'] is visible", async () => {
    try {
    // Missing locator locators.a-href-twitter-com on /; add it to shared locators and rerun generation.
    } finally {
      await captureStepArtifact(page, testInfo, "3. Ensure locator a[href*='twitter.com'] is visible");
    }
  });
});
